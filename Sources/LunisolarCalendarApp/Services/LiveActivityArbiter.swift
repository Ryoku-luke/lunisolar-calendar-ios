...
public enum LiveActivityArbiter {
    ...
    public static func currentOccupant() -> Occupant {
        #if canImport(ActivityKit) && !os(macOS)
        if !Activity<CountdownActivityAttributes>.activities.isEmpty { return .countdown }
        if !Activity<QingheLiveActivityAttributes>.activities.isEmpty { return .timeCapsule }
        return .none
        #else
        return .none
        #endif
