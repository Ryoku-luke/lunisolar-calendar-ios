...
public struct HuangliOverviewWidgetView: View {
    ...
    /// 从 App Group 读主 App 写入的天气缓存（与 WeatherService.store 同步）
    static func sharedWeather() -> (symbol: String, text: String, temperature: String)? {
        guard let def = UserDefaults(suiteName: "group.com.lumingfeng.lunisolarcalendar"),
              let data = def.data(forKey: "weather.cache.v2"),
              let box = try? JSONDecoder().decode(SharedWeatherBox.self, from: data),
              Date().timeIntervalSince(box.snapshot.fetchedAt) < 7200 else { return nil }
        let s = box.snapshot
        let desc = WMOWeather.describe(Int(s.weatherCode))
        return (desc.symbol, desc.text, String(format: "%.0f°", s.temperature))
    }
    ...
    public var body: some View {
        ...
        // P1：Widget 点击 → 打开 App 主界面（日历 Tab）
        .widgetURL(URL(string: "qinghe://calendar"))
    }
    ...
    private var largeView: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // 天气行（从 App Group 读主 App 写入的天气缓存）
            if let w = Self.sharedWeather() {
                HStack(spacing: 6) {
...
/// Widget 侧镜像天气缓存盒（与 WeatherService.WeatherCacheBox 字段一致）
private struct SharedWeatherBox: Codable {
    let coordKey: String
