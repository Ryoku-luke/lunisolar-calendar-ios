...
### 7.1 仍存在的结构性缺口：UI 测试没有数据隔离（建议作为下一步）

**UI 测试跑在用户的真实容器上**，凡是「空数据 / 首次使用」才能成立的断言都不稳定。
Flow 6 原本就吃过这个亏：它早期用「筛选到没有数据的类型」造空态，
容器里提醒与记事都有内容时只能跳过（现在改成搜索造空态，已与环境无关）。

但随着继续补 UI 测试，这个缺口必然再次挡路——例如「首次启动的引导页」「清空数据后的状态」
「导入流程的初始态」都无法稳定构造。
