...
### A. iPad 右栏到底该显示什么（~~两份文档互相矛盾~~ **已裁决 2026-09-26**）
...
- **裁决：上下文 Inspector（UI 报告 §33/§36 方向）**，已落地（2026-09-26）：
  日历 / 年视图节 → 选中日的 `DayDetailView`（原有行为，天然上下文联动）；
  倒数日节 → 新建 `CountdownDetailView` 详情列（列表行点击变为选中，右栏跟随）；
  全部日程 / 设置节 → 右栏整体隐藏（`NavigationSplitView(columnVisibility:)`，等同 Apple 设置）。
  竖屏维持中栏+右栏两列（Flow 4 实测现状），未按总方案 §18 改 sheet。
- 保留一条 UI 测试实测到的事实：iPad **竖屏**下侧栏是**浮层且默认收起**，
  展开后盖住中栏（日期格 `isHittable == false`）；横屏才是三栏并排。
  所以「竖屏优先 Sidebar+Content」（总方案 §18）这个说法与现状不符——
  现状是**竖屏只显示中栏**，侧栏要手动展开。
  Flow 4 已新增 `ipad.inspector.countdown` 断言锁定倒数日节的右栏语义。
