...
- 主 App target 的 **Bundle Identifier**（`com.lumingfeng.lunisolarcalendar`）与 Widget（`com.lumingfeng.lunisolarcalendar.widget`）、App Group（`group.com.lumingfeng.lunisolarcalendar`）需保持一致（见 §7）。
