...
3. メインアプリと Widget 拡張で同じ App Group（`group.com.qinghe.calendar`）を選択していることを確認
...
2. メインアプリと Widget 拡張で同じ App Group（`group.com.qinghe.calendar`）を選択
...
> Linux ではモデル層のみ検証します（旧暦/暦注/イベント CRUD/インポート・エクスポート/同期 Mock）。SwiftUI ビューのコンパイルには iOS/macOS SDK が必要です。CI（`.github/workflows/ci.yml`）では iOS シミュレータでの `xcodebuild test` も実行します。
...
### Bundle ID と App Group（現在値）

| 項目 | 値 |
|---|---|
| メインアプリ Bundle ID | `com.qinghe.calendar` |
| Widget 拡張 Bundle ID | `com.qinghe.calendar.widget` |
...
- [ ] メインアプリ + Widget 拡張で同じ App Group（`group.com.qinghe.calendar`）を選択
