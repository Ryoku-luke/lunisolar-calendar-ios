...
/// 2026 立春 = 2026-02-04 04:01 Asia/Shanghai（紫金山天文台，SolarTermProvider 内置）。
final class YearBoundaryProviderTests: XCTestCase {
    ...
    /// 立春后 1 分钟：2026-02-04 04:01 → 2026 年柱（丙午）
    func testLichun2026AfterBoundaryIsCurrentYear() {
        XCTAssertEqual(YearBoundaryProvider.effectiveGanZhiYear(for: date(2026, 2, 4, 4, 1)), 2026)
        XCTAssertEqual(YearBoundaryProvider.ganZhiString(for: date(2026, 2, 4, 4, 1)), "丙午")
    }
    ...
    /// 2029 超出节气表（2025–2028）→ 退化 2/4 00:00 Asia/Shanghai 近似
    func testFallbackYearUsesFeb4Approximation() {
        XCTAssertEqual(YearBoundaryProvider.effectiveGanZhiYear(for: date(2029, 2, 3, 23, 59)), 2028)
        XCTAssertEqual(YearBoundaryProvider.effectiveGanZhiYear(for: date(2029, 2, 4, 0, 1)), 2029)
    }
    ...
    func testKnownGanZhiAnchor() {
        ...
        // 2024 超出节气表（2025–2028）→ fallback 2/4 00:00 边界，
        // 注意：真实立春为 02-04 16:27，00:00~16:27 窗口会误判为甲辰，
        // 属数据覆盖缺口（见报告"剩余问题"），此处只验证 fallback 行为。
        XCTAssertEqual(YearBoundaryProvider.ganZhiString(for: date(2024, 2, 3, 23, 59)), "癸卯")
        XCTAssertEqual(YearBoundaryProvider.ganZhiString(for: date(2024, 2, 4, 0, 1)), "甲辰")
    }
}
